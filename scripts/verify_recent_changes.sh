...
grep -q '8169449826\|918169449826\|81694 49826' "$ROOT/api/api.php" && ok "Support phone in API default" || bad "Support phone missing"
grep -q 'Malad West' "$ROOT/client/src/constants/siteContact.ts" && ok "Company address defaults (Malad West)" || bad "Company address defaults missing"
grep -q 'Company contact (public website)' "$ROOT/client/src/pages/admin/AdminDashboard.tsx" && ok "Admin Site Settings contact section" || bad "Admin contact settings UI missing"
